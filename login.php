...
if(isset($_POST['Submit']))
{
	...
	echo $userpassword;
	$query1="SELECT * FROM donerrej WHERE user_name = '{$useremail}' || e_mail= '{$useremail}'";
	...
	{
		$dbusername=$row1["user_name"];
		$dbemail=$row1["e_mail"];
		$dbmobile=$row1["mob_ile"];
		$dbquestion=$row1["question1"];
		$dbans=$row1["ans"];
		$dbpass=$row1["pass"];
		$userrole=$row1["user_role"];
		$fullnamedb=$row1["full_name"];
		
		
}

if($useremail && $userpassword){
	if($useremail==$dbemail && $userpassword==$dbpass || $useremail==$dbusername && $userpassword==$dbpass)
		{
			$_SESSION['userrolesess']=$userrole;
			$_SESSION['usernamesess']=$dbusername;
			$_SESSION['passwordsess']=$dbpass;
			$_SESSION['useremailsess']=$useremail;
			$_SESSION['rejnamesess']=$fullnamedb;
			$_SESSION['rejphonesess']=$dbmobile;
			$_SESSION['cstquestionsess']=$dbquestion;
			$_SESSION['answersess']=$dbans;
			header("location: parallelx.php");
			
		}
		else{
			header("Location: home.php?message=Incorrect Username/Password");
	}	
}
else{
	header("Location: home.php?message=Invalid! Username and Password");
}
...
<html>
	<head>
		...
		<title>Login - E-RAKT KOSH MANAGEMENT SYSTEM
</title>
		...
		<div>
			...
			<center style="background-color: aqua;">
				<div style="width: 500px; border: 2px solid #E65100; margin-top: 35px; margin-bottom: 35px; background-color: #FFA726; padding: 20px">
					<img src="images/login2.png" width="400px" height="70px">
					...
					<form name="donor_form" method="post" action="" style="width: 500px; padding: 15px;" enctype="multipart/form-data">
						<table>
							...
							<tr>
								<td>Password:</td>
								<td><input type="password" name="TxtPassword" style="margin-bottom: 10px;font-size: 16px"></td>
							</tr>
